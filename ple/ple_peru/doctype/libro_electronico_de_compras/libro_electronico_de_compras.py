...
class LibroElectronicodeCompras(Utils):
	def get_purchase_invoices(self, year, periodo):
		...
		purchase_invoices = frappe.db.sql("""select      
				...
				IF(base_total_taxes_and_charges=0 OR codigo_comprobante='03', 0, base_net_total - IF(base_inafected_taxes_and_charges=0, 0, base_inafected_taxes_and_charges)) as base_imponible,
				IF(codigo_comprobante!='03',base_total_taxes_and_charges,0) as monto_impuesto,
				"" as base_imponible_exportacion,
				"" as monto_impuesto_exportacion,
				"" as base_imponible_no_gravada,
				"" as monto_impuesto_no_gravada,
				IF(base_total_taxes_and_charges=0, base_grand_total, IF(base_inafected_taxes_and_charges=0, 0, base_inafected_taxes_and_charges)) as valor_adquisicion_no_gravada,
				"" as monto_isc,
				"" as otros_conceptos,
