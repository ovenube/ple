...
def get_sales_invoices(year, periodo):
	...
	sales_invoices = frappe.db.sql("""select
			...
			"" as resumen_diario,
			IF(ISNULL(tax_id),"",IFNULL(codigo_tipo_documento,"")) as tipo_documento,
			IF(ISNULL(tax_id),"",REPLACE(tax_id,"-","")) as numero_documento,
			IF(ISNULL(tax_id),"",IF(customer_name='Clientes Varios',customer_boleta_name,customer_name)) as nombre_cliente,
...
def export_libro_de_ventas(year, periodo, ruc):
	...
	nombre = "LE"+str(ruc)+codigo_periodo+'00140100'+'00'+'1'+'1'+'1'+'1'
